...
export default function MyCourses() {
  ...
  const loadCourses = useCallback(async () => {
    try {
      ...
      setError(
        err.message || "Không thể tải danh sách khóa học. Vui lòng thử lại."
      );
    } finally {
      setLoading(false);
    }
  ...
  const continuePagination = useMemo(() => {
    const sortedByProgress = [...sortedCourses].sort(
      (a, b) => (a.progressPercentage || 0) - (b.progressPercentage || 0)
    );

    const totalPages =
      Math.ceil(sortedByProgress.length / CONTINUE_PER_PAGE) || 1;
    ...
    return {
      totalPages,
      safePage,
      currentCourses: sortedByProgress.slice(
        startIndex,
        startIndex + CONTINUE_PER_PAGE
      ),
    };
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-2 text-sm text-gray-500">
        <span className="text-center sm:text-left">
          Trang {continuePagination.safePage + 1} /{" "}
          {continuePagination.totalPages}
        </span>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
          ...
          <div className="flex items-center gap-1.5">
            {Array.from({ length: continuePagination.totalPages }).map(
              (_, idx) => (
                <button
                  key={idx}
                  onClick={() => setContinuePage(idx)}
                  className={`w-2 h-2 rounded-full transition-all ${
                    continuePagination.safePage === idx
                      ? "bg-yellow-400 w-6"
                      : "bg-gray-300 hover:bg-gray-400"
                  }`}
                />
          ...
          <Button
            ...
            disabled={
              continuePagination.safePage >= continuePagination.totalPages - 1
            }
            className="flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Sau
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Theo dõi tiến độ, tiếp tục học nhanh và nắm bắt hoạt động mới
                  nhất.
                </p>
              </div>
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-10 space-y-3">
                      ...
                      <p className="text-gray-600">
                        Khám phá thêm khóa học để bắt đầu hành trình học tập của
                        bạn.
                      </p>
            ...
            <div className="space-y-6">
              <Card className="shadow-lg border border-amber-50">
                <CardHeader>
                  ...
                  <CardDescription>
                    Thời điểm bạn đăng ký các khóa học.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                  {recentActivities.length === 0 ? (
                    <p className="text-gray-500 text-sm text-center">
                      Chưa có hoạt động nào. Bắt đầu đăng ký khóa học để thấy
                      lịch sử ở đây.
                    </p>
                  ) : (
                ...
                <CardContent className="space-y-4 text-sm">
                  <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100">
                    ...
                    <p className="text-amber-800 mt-1">
                      Lên lịch học cố định để giữ nhịp độ và hoàn thành khóa học
                      đúng hạn.
                    </p>
                  </div>
                  <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                    ...
                    <p className="text-indigo-800 mt-1">
                      Tổng hợp ghi chú giúp bạn nắm chắc kiến thức và dễ dàng ôn
                      luyện.
                    </p>
                  </div>
                </CardContent>
