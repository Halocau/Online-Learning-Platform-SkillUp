...
namespace SkillUp.Controllers
{
	...
	public class AuthController : ControllerBase
	{
		...
		[HttpPost("register-mod")]
		[Authorize]
		public async Task<IActionResult> RegisterForMods([FromBody] RegisterRequestDto request)
		{
			try
			{
				...
				if (request.RoleId != 2 && request.RoleId != 3)
				{
					...
					{
						code = 400,
						message = "Chức vụ không hợp lệ cho Moderator!",
						data = new List<object>()
					});
				}
				...
				// Call service register for mods
				var success = await _authService.RegisterForModsAsync(request);
				// Registration failed (email exists)
				if (!success)
				{
					return BadRequest(new APIReturn
				...
				{
					code = 200,
					message = "Đăng ký tài khoản Moderator thành công.",
					data = new List<object>()
				});
			}
			catch (Exception ex)
