...
namespace SkillUp.Services.Implementations
{
    public class EmailService : Interfaces.IEmailService
    {
        ...
        public async Task<bool> SendStatusEmailAsync(string toEmail, string fullname, string status, string customBody)
        {
            var message = new MimeMessage();
            message.From.Add(new MailboxAddress("SkillUp Platform", _fromEmail));
            message.To.Add(new MailboxAddress(fullname, toEmail));
            message.Subject = $"Thông báo trạng thái đơn đăng ký - {status}";

            var bodyBuilder = new BodyBuilder { HtmlBody = customBody };
            message.Body = bodyBuilder.ToMessageBody();
