...
namespace SkillUp.Services.Interfaces
{
	public interface IQuestionBankService
	{
		...
		Task<List<SectionQuestionBankDTO>> GetQuestionBanksByCourseIdAsync(Guid accountId, Guid courseId);
		Task<DetailQuestionBankDTO> GetQuestionBankByIdAsync(Guid id, Guid accountId, Guid courseId);
		Task<DetailQuestionBankDTO> CreateQuestionBankAsync(CreateQuestionBankDTO createQuestionBankDTO, Guid accountId, Guid courseId, string? imageUrl);
		...
		Task<List<CreateQuestionBankResponseDTO>> ReadQuestionsWithMultipleAnswersAsync(Stream excelStream, Guid sectionId, Guid accountId);
